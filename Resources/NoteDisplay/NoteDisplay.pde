...
void setup ()
{
  ...
  {
    notes.add(new Note(int(reader.next()), reader.next(), int(reader.next()), int(reader.next())));
  }
  reader.close();
...
void draw ()
{
  ...
  for (Note note : notes)
  {
    text(note.name, ((note.startTime - minTime) / (totalLength + 0.0)) * width, height - (((note.pitch - minPitch) / (pitchRange + 0.0)) * height) + 12);
  }
  noLoop();
}
